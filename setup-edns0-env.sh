...
#   ./setup-edns0-env.sh --persist       # grava config não-interativa em /etc
#   ./setup-edns0-env.sh --verify        # só roda as verificações finais
#
# O script já roda sem prompts por padrão (variáveis de ambiente, válidas
# só para este processo). A flag --persist estende isso a toda a máquina,
# alterando /etc/apt, /etc/environment e /etc/needrestart — apropriado
# apenas em VM descartável.
#
...
# Evita telas interativas do debconf (Postfix, reinício de serviços etc.).
# Necessário porque a saída deste script passa por 'tee', o que corrompe o
# desenho da interface ncurses e deixa o diálogo impossível de operar.
export DEBIAN_FRONTEND=noninteractive
export NEEDRESTART_MODE=a
export NEEDRESTART_SUSPEND=1

preseed_debconf() {
  say "Pré-configurando debconf (sem telas interativas)"
  $SUDO apt-get install -y -qq debconf-utils >/dev/null 2>&1 || true

  # O Zeek/ZeekControl puxa um MTA. Não precisamos de e-mail nesta avaliação.
  # Sem este preseed, o modo noninteractive aceitaria o padrão do pacote
  # ("Internet Site"), que não é o que queremos.
  printf '%s\n' \
    'postfix postfix/main_mailer_type select No configuration' \
    'postfix postfix/mailname string localhost' \
    | $SUDO debconf-set-selections
  ok "Postfix marcado como 'No configuration'"

  [ "$PERSIST_CONFIG" -eq 1 ] || return 0

  say "Aplicando configuração não-interativa PERMANENTE no sistema"
  warn "Isto altera /etc — use apenas em VM descartável de avaliação."

  # apt: assume 'sim' e mantém arquivos de configuração existentes.
  # --force-confold cobre o prompt 'manter ou substituir config?', que
  # também é ncurses e também trava quando a saída passa por tee.
  $SUDO tee /etc/apt/apt.conf.d/90noninteractive >/dev/null <<'EOF'
APT::Get::Assume-Yes "true";
Dpkg::Options { "--force-confold"; "--force-confdef"; };
EOF
  ok "/etc/apt/apt.conf.d/90noninteractive criado"

  # Frontend padrão do debconf para sessões futuras (ssh, reboot).
  if ! grep -qs '^DEBIAN_FRONTEND=' /etc/environment; then
    echo 'DEBIAN_FRONTEND=noninteractive' | $SUDO tee -a /etc/environment >/dev/null
    ok "DEBIAN_FRONTEND fixado em /etc/environment"
  else
    ok "DEBIAN_FRONTEND já presente em /etc/environment"
  fi

  # needrestart: reinicia serviços sem perguntar (Ubuntu 22.04+).
  if [ -f /etc/needrestart/needrestart.conf ]; then
    $SUDO sed -i "s/^#\?\$nrconf{restart}.*/\$nrconf{restart} = 'a';/" \
      /etc/needrestart/needrestart.conf && ok "needrestart em modo automático"
  fi
}
