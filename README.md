...
This repository accompanies the proof-of-concept notebook submitted to **SBSEG 2026**. The work presents an **LLM-driven pipeline** that automates the generation of intrusion detection rules for IEC 61850 digital substations using the **GOOSE** protocol.
...
```
.
├── SBSEG_2026_LLM_IDS_GOOSE_v1.ipynb # Main notebook (proof of concept)
...
git clone https://github.com/lucastuxnet/SBSEG_2026.git
cd SBSEG_2026
...
jupyter notebook SBSEG_2026_LLM_IDS_GOOSE_v1.ipynb
```
...
```bash
docker build -t sbseg2026 .
```

#### 5. Run the Container (without persistence)

```bash
docker run -p 8888:8888 --env-file .env sbseg2026
```
...
```
```bash
docker run -p 8888:8888 --env-file .env -v $(pwd)/output_do_projeto:/app sbseg2026
```
...
Open this URL in your browser and click on `SBSEG_2026_LLM_IDS_GOOSE_v1.ipynb`.
...
| Module not found errors | Rebuild image with `docker build --no-cache -t sbseg2026 .` |
...
|---------|-----|-------------|------|
| v1.2 | `sbseg2026-submission` | Initial submission version | May 2026 |
...
If you use this work, please cite the corresponding paper published at **SBSEG 2026**.
